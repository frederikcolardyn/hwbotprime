...
public class VersionCheckTask extends AsyncTask<Void, Void, Void> {
    ...
    public Void doInBackground(Void... params) {
        ...
        try {
            URL url = new URL(BenchService.SERVER + "/version/api?client=" + URLEncoder.encode(OpenGLBenchService.HWBOT_PRIME_APP_NAME, "utf8") + "&os=android");
            ...
            } else if (apiResponse.getVersion() != null && apiResponse.getVersion().equals(OpenGLBenchService.getInstance().version)) {
                // all ok!
                Log.i(this.getClass().getSimpleName(), "Latest version, all ok!");
            } else if (apiResponse.getSupportedVersions() != null && apiResponse.getSupportedVersions().contains(OpenGLBenchService.getInstance().version)) {
